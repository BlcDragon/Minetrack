...
const graphDuration = config.graphDuration * 1000;
...
class Database {
    ...
    getRecord(ip, callback) {
        const table = config.saveMyHdd ? 'pings' : 'records';
    ...
    _clearOldPings() {
        ...
        const statement = db.prepare('DELETE FROM pings WHERE timestamp<?')
        statement.run(timestamp, err => {
            if (err) {
    ...
    _isDbClearReady () {
        return TimeTracker.getEpochMillis() - this._lastDbClear <= 3600000
    }
}
